           ...
           const user=await Auth.update(
            ...
            if(user)
            {
                t.commit()
                 ...
                 res.status(201).json({expenseData:expense.dataValues})
            }
             else{
                t.rollback()
...
exports.getExpenses=async(req,res,next)=>{
    try{
    const expenses= await req.user.getExpenses()
    const totalPages=Math.floor((expenses.length)/5);
   
    
     res.status(201).json({expenseData:expenses,totalPages:totalPages})
    }
    catch(error){
        console.log(error) 
    }
